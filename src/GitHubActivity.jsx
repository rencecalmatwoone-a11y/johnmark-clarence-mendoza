...
const CalendarGrid = memo(function CalendarGrid({ days, onSelect }) {
  ...
  const { ref: scrollRef, play: playVisibleHover } = useVisibleSound(playHover)
          ...
          <div className="github-days" onMouseLeave={() => onSelect(null)}>
            {days.map((day, index) => (
              <button
                ...
                title={labels[index]}
                onMouseEnter={() => { onSelect(day); playVisibleHover(day.level, index / Math.max(1, days.length - 1) * 2 - 1) }}
                onFocus={() => { setFocused(index); onSelect(day); playVisibleHover(day.level) }}
                onBlur={() => onSelect(null)}
                onClick={() => { onSelect(day); playVisibleHover(day.level) }}
